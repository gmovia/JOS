...
mem_init(void)
{
	...
	boot_map_region(kern_pgdir, KERNBASE, pa_end + 1, 0, PTE_W);
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref != 0 || pp->pp_link != NULL)
		panic("trying to free a page in use");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (*dir_entry & PTE_P) {
		...
		if (create) {
			...
			if (page) {
				page->pp_ref++;
				*dir_entry = page2pa(page) | PTE_U | PTE_W | PTE_P;
				page_table = KADDR(PTE_ADDR(*dir_entry));
				return &page_table[PTX(va)];
			}
		}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// the space should be aligned
	if (va % PGSIZE != 0 || pa % PGSIZE != 0 || size % PGSIZE != 0)
		panic("mapping unaligned space.");

#ifndef TP1_PSE
	for (int i = 0; i < size; i += PGSIZE) {
		pte_t *pt_e = pgdir_walk(pgdir, (void *) (va + i), true);
		if (pt_e == NULL)
			panic("out of memory.");
		*pt_e = (pa + i) | perm | PTE_P;
	}
#else
	for (int i = 0; i < size; i += PGSIZE) {
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if (*pt_e & PTE_P && PTE_ADDR(*pt_e) != page2pa(pp)) {
		page_remove(pgdir, va);
	}
	if (PTE_ADDR(*pt_e) != page2pa(pp))
		pp->pp_ref++;
...
{
	// pointer where lookup will store the pt entry
	pte_t *pt_e;
...
{
	void *va_round = ROUNDDOWN((void *) va, PGSIZE);
	int n = PGNUM(ROUNDUP(va + len, PGSIZE)) - PGNUM(va_round);
	for (int i = 0; i < n; i++) {
		uint32_t va_check = (uint32_t) va_round + (i * PGSIZE);
		pte_t *pt_e = pgdir_walk(env->env_pgdir, (void *) va_check, 0);
		if (pt_e && (*pt_e & (perm | PTE_P)) == (perm | PTE_P)) {
			if (va_check >= ULIM) {
				if (i == 0)
			...
			if (i == 0)
				...
				user_mem_check_addr = va_check + (i * PGSIZE);
			return -E_FAULT;
		}
	}
